...
def create_diagram():
    ...
    diagram = ET.SubElement(mxfile, "diagram", id="clean_attendance_config", name="Attendance Configuration")
    ...
    # Controller
    c_ctrl = add_class("AttendanceConfigController", "RestController", ["- configService : AttendanceConfigService"], ["+ getConfig() : ResponseEntity&lt;AttendanceConfig&gt;", "+ updateConfig(AttendanceConfig) : ResponseEntity&lt;AttendanceConfig&gt;"], x=200, y=50, width=400)

    # Interface
    c_svc = add_class("AttendanceConfigService", "interface", [], ["+ getConfig() : AttendanceConfig", "+ updateConfig(AttendanceConfig) : AttendanceConfig"], x=200, y=250, width=400)

    # Service Impl
    c_svc_impl = add_class("AttendanceConfigServiceImpl", "Service", ["- configRepository : AttendanceConfigRepository", "- systemLogService : SystemLogService", "- DEFAULT_CONFIG_KEY : String = \"SYSTEM_CONFIG\""], ["+ getConfig() : AttendanceConfig", "+ updateConfig(AttendanceConfig) : AttendanceConfig"], x=200, y=450, width=400)

    # SystemLogService
    c_syslog = add_class("SystemLogService", "Service", [], ["+ logAttendanceConfigUpdated() : void"], x=650, y=500, width=300)

    # Repository
    c_repo = add_class("AttendanceConfigRepository", "interface", [], ["+ findByConfigKey(String) : Optional&lt;AttendanceConfig&gt;"], x=200, y=700, width=400)

    # Entity
    c_ent = add_class("AttendanceConfig", "Entity", ["- id : Long", "- configKey : String", "- manualEnabled : Boolean", "- absentThresholdMinutes : Integer", "- minAttendancePercentage : Double", "- faceRecognitionEnabled : Boolean", "- maxAttempts : Integer", "- wifiLocationEnabled : Boolean", "- createdAt : LocalDateTime", "- updatedAt : LocalDateTime"], [], x=200, y=900, width=400)

    # Relations
    add_edge(c_ctrl, c_svc, "uses", "dashed")
    add_edge(c_svc_impl, c_svc, "implements", "implements")
    add_edge(c_svc_impl, c_repo, "uses", "dashed")
    add_edge(c_svc_impl, c_syslog, "uses", "dashed")
    add_edge(c_repo, c_ent, "manages", "dashed")

    tree = ET.ElementTree(mxfile)
    tree.write("../docs/attendance_configuration_class_diagram.drawio", encoding="UTF-8", xml_declaration=False)

if __name__ == "__main__":
    create_diagram()
    print("Clean Attendance Configuration diagram generated successfully!")
