...
def create_diagram():
    ...
    diagram = ET.SubElement(mxfile, "diagram", id="clean_enrollment", name="Enrollment Management")
    ...
    # DTOs
    c_dto_req = add_class("EnrollmentRequest", "DTO", ["- className : String", "- studentCode : String", "- status : String"], [], x=150, y=50, width=280)
    c_dto_res = add_class("EnrollmentResponse", "DTO", ["- id : Long", "- studentCode : String", "- studentName : String", "- className : String", "- status : EnrollmentStatus"], [], x=550, y=50, width=280)

    # Controller
    c_ctrl = add_class("ClassSectionController", "RestController", ["- classSectionService : ClassSectionService", "- stagingImportService : StagingImportService"], ["+ getEnrollmentsByClassName(className) : List", "+ createEnrollment(EnrollmentRequest) : EnrollmentResponse", "+ updateEnrollment(...) : EnrollmentResponse", "+ deleteEnrollment(enrollmentId) : void", "+ deleteEnrollments(enrollmentIds) : void", "+ getAvailableClassSectionsForTransfer(...) : List", "+ getAvailableClassSectionsForTransferWithConflict(...) : List", "+ transferEnrollments(...) : void", "+ getEnrollmentImportTemplate(...) : byte[]", "+ fastPreviewEnrollments(...) : Map", "+ bulkImportEnrollments(...) : Map"], x=250, y=250, width=480)

    # Service
    c_svc = add_class("ClassSectionService", "Service", ["- enrollmentRepo : EnrollmentRepository", "- classSectionRepo : ClassSectionRepository", "- userRepo : UserRepository"], ["+ getEnrollmentsByClassName(className) : List", "+ createEnrollment(EnrollmentRequest) : EnrollmentResponse", "+ updateEnrollment(...) : EnrollmentResponse", "+ deleteEnrollment(enrollmentId) : void", "+ deleteEnrollments(enrollmentIds) : void", "+ getAvailableClassSectionsForTransfer(...) : List", "+ getAvailableClassSectionsForTransferWithConflict(...) : List", "+ transferEnrollments(...) : void", "+ getEnrollmentImportTemplate(...) : byte[]"], x=270, y=600, width=440)

    # Repository
    c_repo = add_class("EnrollmentRepository", "interface", [], ["+ findByClassSectionId(Long) : List", "+ findByStudentId(Long) : List", "+ existsByStudentIdAndClassSectionId(...) : boolean"], x=290, y=950, width=400)

    # Entities
    c_ent_enrol = add_class("Enrollment", "Entity", ["- id : Long", "- student : User", "- classSection : ClassSection", "- enrolledAt : LocalDateTime", "- status : EnrollmentStatus"], [], x=450, y=1150, width=300)
    c_ent_class = add_class("ClassSection", "Entity", ["- id : Long", "- className : String", "- course : Course", "- semester : Semester"], [], x=100, y=1150, width=280)
    ...
    # Service -> Repos
    add_edge(c_svc, c_repo, "uses", "dashed")
    
    # Repos -> Entities
    add_edge(c_repo, c_ent_enrol, "manages", "dashed")

    # Entities Relation
    add_edge(c_ent_enrol, c_ent_class, "*..1", "association")

    tree = ET.ElementTree(mxfile)
    tree.write("../docs/enrollment_management_class_diagram.drawio", encoding="UTF-8", xml_declaration=False)

if __name__ == "__main__":
    create_diagram()
    print("Clean Enrollment Management diagram generated successfully!")
