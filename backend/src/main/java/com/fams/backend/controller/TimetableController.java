...
public class TimetableController {
        ...
        public ResponseEntity<TimetableDTO.GenerateResponse> generateTimetable(
                ...
                log.info("Starting timetable generation for semester: {}",request.getSemesterCode());log.info("Starting timetable generation for semester: {}",request.getSemesterCode());
                ...
                String jobId=UUID.randomUUID().toString();CompletableFuture<TimetableGenerationService.GenerationResult>future=generationService.generateTimetable(jobId,request.getSemesterCode(),config,null);
                // Start async generation
                String jobId=UUID.randomUUID().toString();CompletableFuture<TimetableGenerationService.GenerationResult>future=generationService.generateTimetable(jobId,request.getSemesterCode(),config,null);

                // Wait for result (or use async approach with job tracking)
                try{TimetableGenerationService.GenerationResult result=future.get();
                // Wait for result (or use async approach with job tracking)
                try{TimetableGenerationService.GenerationResult result=future.get();

                return ResponseEntity.ok(TimetableDTO.GenerateResponse.builder().success(result.isSuccess()).jobId(result.getJobId()).message(result.getMessage()).fitness(result.getFitness()).totalGenerations(result.getTotalGenerations()).durationMs(result.getDurationMs()).totalSlots(result.getTotalSlots()).totalClasses(result.getTotalClasses()).build());}catch(Exception e){log.error("Error generating timetable",e);return ResponseEntity.internalServerError().body(TimetableDTO.GenerateResponse.builder().success(false).message("Error: "+e.getMessage()).build());}}return ResponseEntity.ok(TimetableDTO.GenerateResponse.builder().success(result.isSuccess()).jobId(result.getJobId()).message(result.getMessage()).fitness(result.getFitness()).totalGenerations(result.getTotalGenerations()).durationMs(result.getDurationMs()).totalSlots(result.getTotalSlots()).totalClasses(result.getTotalClasses()).build());
        }catch(
        ...
        public ResponseEntity<Map<String, String>> startAsyncGeneration(
                ...
                return ResponseEntity.accepted()
                                .body(Map.of(
                                                ...
                                                "status", "RUNNING"));
        }return ResponseEntity.accepted().body(Map.of("jobId",jobId,"message","Timetable generation started","status","RUNNING"));
        ...
        public ResponseEntity<TimetableDTO.JobStatusResponse> getJobStatus(@PathVariable String jobId) {
                ...
                return ResponseEntity.ok(TimetableDTO.JobStatusResponse.builder()
                                ...
                                .build());
        }return ResponseEntity.ok(TimetableDTO.JobStatusResponse.builder().jobId(job.getJobId()).semesterCode(job.getSemesterCode()).status(job.getStatus().name()).phase(job.getPhase()).currentGeneration(job.getCurrentGeneration()).bestFitness(job.getBestFitness()).percentComplete(job.getPercentComplete()).startTime(job.getStartTime()).endTime(job.getEndTime()).errorMessage(job.getErrorMessage()).build());
        ...
        }return ResponseEntity.ok(Map.of("jobId",jobId,"cancelled",cancelled,"message",cancelled?"Job cancelled":"Job not found or already completed"));
        ...
        }return ResponseEntity.ok(Map.of("unscheduledCount",unscheduledCount,"totalSchedulable",totalSchedulable,"scheduledCount",scheduledCount,"unscheduledClassNames",unscheduledClassNames));
        ...
        }return ResponseEntity.ok(Map.of("configChanged",configChanged,"hasTimetable",true,"timetableCreatedAt",timetableCreatedAt.toString(),"configUpdatedAt",configUpdatedAt!=null?configUpdatedAt.toString():"null","message",configChanged?"Cấu hình học kỳ đã thay đổi. Vui lòng tạo thời khóa biểu mới.":"Semester config is up to date"));
                        ...
                        if (!slots.isEmpty()) {
                                ...
                                response.getDays().forEach(day -> day.getSlots().forEach(slotDto -> {
                                        ...
                                        if (assignment != null) {
                                                ...
                                                slotDto.setSubmissionStatus(subStatus != null ? subStatus : "NOT_SUBMITTED");
                                        }
                                }));
                        }
        ...
        Exception e){log.error("CRITICAL ERROR in getStudentTimetable for student "+studentId,e);throw e;
        }}
        ...
        }log.info("Returning {} slots for student {} in semester {}",slotDTOs.size(),studentId,semesterCode);return ResponseEntity.ok(slotDTOs);
        ...
        // 4. Generate Excel
        excelExportService.exportLecturerScheduleToExcel(response,slotDTOs,lecturer.getFullName(),semester.getName());
        ...
        public ResponseEntity<TimetableDTO.TimetableStatsDTO> getTimetableStats(
                ...
                return ResponseEntity.ok(TimetableDTO.TimetableStatsDTO.builder()
                                ...
                                .build());
        }return ResponseEntity.ok(TimetableDTO.TimetableStatsDTO.builder().semesterCode(semesterCode).totalSlots((int)totalSlots).totalClasses((int)totalClasses).saturdaySlots((int)saturdaySlots).build());
...
private GAConfig convertToGAConfig(TimetableDTO.GAConfigDTO dto) {
                if(dto==null){return GAConfig.defaultConfig();
        }
...
private TimetableDTO.TimetableSlotDTO convertToDTO(TimetableSlot slot) {
        ...
        private TimetableDTO.TimetableSlotDTO convertToDTO(TimetableSlot slot) {
                ...
                return TimetableDTO.TimetableSlotDTO.builder()
                                ...
                                .build();
        }return TimetableDTO.TimetableSlotDTO.builder().id(slot.getId()).classSectionId(cs!=null?cs.getClassName():null).className(cs!=null?cs.getClassName():null).courseCode(course!=null?course.getCode():null).courseName(course!=null?course.getName():null).lecturerName(lecturer!=null?lecturer.getFullName():null).roomCode(room!=null?room.getCode():null).roomName(room!=null?room.getName():null).date(slot.getDate()).dayOfWeek(slot.getDayOfWeek()).slotNumber(slot.getSlotNumber()).startTime(slotType!=null?slotType.getStartTime():null).endTime(slotType!=null?slotType.getEndTime():null).status(slot.getStatus()!=null?slot.getStatus().name():null).build();
...
private TimetableDTO.WeeklyTimetableDTO buildWeeklyTimetable(
        ...
        private TimetableDTO.WeeklyTimetableDTO buildWeeklyTimetable(
                ...
                return TimetableDTO.WeeklyTimetableDTO.builder()
                                ...
                                .days(days)
                                .build();
        }return TimetableDTO.WeeklyTimetableDTO.builder().weekStartDate(weekStart).weekEndDate(weekEnd).days(days).build();
}}
