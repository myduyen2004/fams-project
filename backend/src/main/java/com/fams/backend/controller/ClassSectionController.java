...
public class ClassSectionController {
    ...
    public ResponseEntity<Page<ClassSectionResponse>> getClassSectionsBySemester(
        ...
        log.info("GET /api/v1/class-sections/semester/{} | search={}, status={}, lecturerId={}, page={}, size={}",
                semesterCode, search, status, lecturerId, pageable.getPageNumber(), pageable.getPageSize());
    ...
    @PostMapping("/semester/{semesterCode}/import/preview")
    @Operation(summary = "Preview import class sections", description = "Preview and validate class sections from Excel file before importing")
    public ResponseEntity<List<ClassSectionImportDTO>> previewImportClassSections(
            @PathVariable String semesterCode,
            @RequestParam("file") MultipartFile file) {
        log.info("POST /api/v1/class-sections/semester/{}/import/preview | filename={}",
                semesterCode, file.getOriginalFilename());
        return ResponseEntity.ok(classSectionService.previewImportClassSections(semesterCode, file));
    }

    @PostMapping("/semester/{semesterCode}/import/save")
    @Operation(summary = "Save imported class sections", description = "Save validated class sections to database")
    public ResponseEntity<Map<String, Object>> saveImportedClassSections(
            @PathVariable String semesterCode,
            @RequestBody List<ClassSectionImportDTO> dtos) {
        log.info("POST /api/v1/class-sections/semester/{}/import/save | count={}", semesterCode, dtos.size());
        return ResponseEntity.ok(classSectionService.saveImportedClassSections(semesterCode, dtos));
    }
    ...
    public ResponseEntity<byte[]> getImportTemplate() {
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=class_sections_import_template.xlsx")
                .contentType(MediaType.APPLICATION_OCTET_STREAM)
                .body(template);
    }

    // ==================== ENROLLMENT IMPORT ENDPOINTS (Multi-class support)
    // ====================

    @PostMapping("/semester/{semesterCode}/enrollments/import/preview")
    @Operation(summary = "Preview import enrollments", description = "Preview and validate enrollments from Excel file for multiple class sections")
    public ResponseEntity<List<EnrollmentImportDTO>> previewImportEnrollments(
            @PathVariable String semesterCode,
            @RequestParam("file") MultipartFile file) {
        log.info("POST /api/v1/class-sections/semester/{}/enrollments/import/preview | filename={}",
                semesterCode, file.getOriginalFilename());
        return ResponseEntity.ok(classSectionService.previewImportEnrollments(semesterCode, file));
    }

    @PostMapping("/enrollments/import/save")
    @Operation(summary = "Save imported enrollments", description = "Save validated enrollments to database for multiple class sections")
    public ResponseEntity<Map<String, Object>> saveImportedEnrollments(
            @RequestBody List<EnrollmentImportDTO> dtos) {
        log.info("POST /api/v1/class-sections/enrollments/import/save | count={}", dtos.size());
        return ResponseEntity.ok(classSectionService.saveImportedEnrollments(dtos));
    }
    ...
    public ResponseEntity<byte[]> getEnrollmentImportTemplate(@PathVariable String semesterCode) {
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION,
                        "attachment; filename=enrollment_import_template_" + semesterCode + ".xlsx")
                .contentType(MediaType.APPLICATION_OCTET_STREAM)
                .body(template);
    }
}
