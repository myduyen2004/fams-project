...
public class StudentGradeServiceTest {
        ...
        class StudentGradeVisibility {
                        ...
                        response.getGradeCategories().stream()
                                        ...
                                        .forEach(item -> {
                                                assertNull(item.getValue(),
                                                                "Quiz score phải là null khi chưa submitted");
                        ...
                        response.getGradeCategories().stream()
                                        ...
                                        .forEach(item -> {
                                                assertNotNull(item.getValue(),
                                                                "Quiz score phải hiển thị sau khi submitted");
                ...
                void shouldShowExamGradesAfterPublished() {
                        ...
                        response.getGradeCategories().stream()
                                        ...
                                        .filter(i -> "Final Exam".equals(i.getItemName()))
                                        .forEach(item -> {
                                                assertEquals(4.0, item.getValue(),
                                                                "FE score phải hiển thị sau khi published");
                                        });
                }
                ...
                void shouldHideResitGradesWhenNotPublished() {
                        ...
                        response.getGradeCategories().stream()
                                        ...
                                        .forEach(item -> {
                                                assertNull(item.getValue(),
                                                                "Resit score phải null khi chưa resitPublished");
                                        });
                }
                ...
                void shouldShowResitGradesAfterPublished() {
                        ...
                        response.getGradeCategories().stream()
                                        ...
                                        .filter(i -> "Resit".equals(i.getItemName()))
                                        .forEach(item -> {
                                                assertEquals(6.5, item.getValue(),
                                                                "Resit score phải hiển thị sau khi resitPublished");
                                        });
                }
        }
        ...
        class GradeOverviewPermissions {
                ...
                void academicStaff_shouldHideGradesWhenNotSubmitted() {
                        ...
                        response.getStudentGrades().forEach(row -> {
                                assertTrue(row.getGrades().isEmpty()
                                                || row.getGrades().values().stream().allMatch(Objects::isNull),
                ...
                void lecturer_shouldAlwaysSeeGrades() {
                        ...
                        assertFalse(response.getStudentGrades().isEmpty());
                        response.getStudentGrades().forEach(row -> {
                                assertFalse(row.getGrades().isEmpty(),
                                                "Giảng viên phải thấy điểm kể cả khi chưa submitted");
                        });
                }
        }
