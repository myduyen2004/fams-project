-- Script sinh dữ liệu điểm số mẫu cho tất cả sinh viên dựa trên Enrollments và Grade Components
BEGIN;
-- 0. Xóa tất cả điểm hiện có và reset trạng thái của lớp học
...
-- 1. Tạo điểm ngẫu nhiên cho các thành phần điểm chính (không phải thi lại)
...
-- 2. Tạo điểm thi lại (chỉ dành cho sinh viên có điểm thi chính < 4.0)
...
-- 2. Cập nhật trạng thái đã nộp điểm cho các lớp đã có điểm
