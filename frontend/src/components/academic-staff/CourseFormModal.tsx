...
interface CourseFormModalProps {
    ...
    onClose: () => void;
    onSuccess: () => void;
    course?: Course | null; // null = create, Course = edit
}
    ...
    useEffect(() => {
        if (course) {
            setFormData({
                ...
                fixedSemester: course.fixedSemester
            });
        } else {
            setFormData({ code: '', name: '', description: '', credits: 3, numberOfSlots: 45, fixedSemester: 1 });
        }
    }, [course, isOpen]);
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" value={formData.code} onChange={(e) => setFormData(prev => ({ ...prev, code: e.target.value }))}
                                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-fpt-orange focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-white" placeholder="VD: PRF192" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1">Số tín chỉ *</label>
                            <input type="number" value={formData.credits} onChange={(e) => setFormData(prev => ({ ...prev, credits: parseInt(e.target.value) || 0 }))}
                                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-fpt-orange focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-white" min={1} max={10} />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1">Số slot</label>
                            <input type="number" value={formData.numberOfSlots} onChange={(e) => setFormData(prev => ({ ...prev, numberOfSlots: parseInt(e.target.value) || 0 }))}
                                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-fpt-orange focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-white" min={1} />
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1">Tên môn học *</label>
                        <input type="text" value={formData.name} onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))}
                            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-fpt-orange focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-white" placeholder="VD: Programming Fundamentals" />
                    </div>
                    <div>
