...
const MessagesPage: React.FC<MessagesPageProps> = ({ role }) => {
    ...
    const timeoutSubsRef = useRef<{ [key: string]: ReturnType<typeof setTimeout> }>({});
    ...
    const [previewData, setPreviewData] = useState<{ isOpen: boolean, url: string | null, name: string | null, type: string | null } | null>(null);
    ...
    const triggerMarkAsRead = (groupId: number) => {
        ...
        setGroups(prev => prev.map(g => g.id === groupId ? { ...g, unreadCount: 0, firstUnreadMessageId: undefined } : g));
        ...
        const stompClient = new Client({
            ...
            onConnect: () => {
                ...
                allGroups.forEach(group => {
                    ...
                    if (stompSubsRef.current[msgId]) stompSubsRef.current[msgId].unsubscribe();
                    stompSubsRef.current[msgId] = stompClient.subscribe(`/topic/chat/${group.id}`, (message) => {
                    ...
                    if (stompSubsRef.current[readId]) stompSubsRef.current[readId].unsubscribe();
                    stompSubsRef.current[readId] = stompClient.subscribe(`/topic/chat/${group.id}/read`, (message) => {
                        ...
                        setMessages(prev => {
                            ...
                            const updated = prev.map(m => {
                                const isMatch = messageIds.some((id: string | number) => Number(id) === Number(m.id));
                                if (isMatch) {
                    ...
                    if (stompSubsRef.current[deleteId]) stompSubsRef.current[deleteId].unsubscribe();
                    stompSubsRef.current[deleteId] = stompClient.subscribe(`/topic/chat/${group.id}/delete`, (message) => {
                    ...
                    if (stompSubsRef.current[reactionId]) stompSubsRef.current[reactionId].unsubscribe();
                    stompSubsRef.current[reactionId] = stompClient.subscribe(`/topic/chat/${group.id}/reaction`, (message) => {
                        const updatedMsg = JSON.parse(message.body) as ChatMessageResponse;
                ...
                if (stompSubsRef.current[userNotificationsId]) stompSubsRef.current[userNotificationsId].unsubscribe();
                stompSubsRef.current[userNotificationsId] = stompClient.subscribe('/user/queue/chat-notifications', (message) => {
                    const data = JSON.parse(message.body);
    ...
    const setupGroupSubscriptions = (client: Client, groupId: number) => {
        ...
        if (stompSubsRef.current[typingId]) stompSubsRef.current[typingId].unsubscribe();
        stompSubsRef.current[typingId] = client.subscribe(`/topic/chat/${groupId}/typing`, (message) => {
            const data = JSON.parse(message.body);
            const user = JSON.parse(localStorage.getItem('user') || '{}');
    ...
    useEffect(() => {
        ...
        if (selectedGroup) {
            ...
            if (stompClientRef.current?.connected) {
                ...
                if (!selectedGroup.unreadCount || selectedGroup.unreadCount <= UNREAD_THRESHOLD) {
                    stompClientRef.current.publish({
    ...
    const loadMessages = async (groupId: number) => {
        try {
            ...
            if (!selectedGroupRef.current?.unreadCount || selectedGroupRef.current.unreadCount <= UNREAD_THRESHOLD) {
                try {
                    console.log('[MessagesPage] Unread <= threshold, marking group as read automatically', groupId);
                ...
                console.log('[MessagesPage] Unread > threshold, waiting for user to scroll/click icon');
            }
    ...
    useLayoutEffect(() => {
        if (messages.length > 0) {
            ...
            if (isInitialLoad.current && selectedGroup?.unreadCount && selectedGroup.unreadCount > UNREAD_THRESHOLD && selectedGroup?.firstUnreadMessageId) {
                const unreadEl = messageRefs.current[selectedGroup.firstUnreadMessageId];
                if (unreadEl) {
                    console.info('[MessagesPage] Unread count > threshold, stopping at boundary:', selectedGroup.firstUnreadMessageId);
                    ...
                    console.info('[MessagesPage] Unread count > threshold but element not found. Staying at top.');
                    setUnreadMessageCount(selectedGroup.unreadCount);
                    setShowScrollBottom(true);
                    isInitialLoad.current = false;
                    return;
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [messages.length, selectedGroup?.id, selectedGroup?.firstUnreadMessageId, selectedGroup?.unreadCount]);
    ...
    return (
        <div className="flex h-[calc(100vh-80px)] bg-gray-50 overflow-hidden">
            {/* Sidebar - Group List */}
            <div
                className={`${selectedGroup ? 'hidden md:flex' : 'flex'} flex-col relative bg-white border-r border-gray-100 transition-[width] duration-75`}
                style={{ width: selectedGroup && window.innerWidth < 768 ? '100%' : `${leftSidebarWidth}px` }}
            >
                <div
                ...
                <div className="p-4 pb-2">
                    <h1 className="text-sm font-bold text-gray-400 mb-4 tracking-[0.2em] uppercase px-1">Tin nhắn nhóm</h1>
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            {(() => {
                                                ...
                                                if (avatars.length === 0) {
                                                    return (
                                                        <div className={`w-11 h-11 ${isSelected ? 'bg-orange-100' : 'bg-orange-50'} rounded-full flex items-center justify-center flex-shrink-0 border border-transparent`}>
                                                            <Users className={isSelected ? 'text-orange-600' : 'text-orange-400'} size={20} />
                                                        </div>
                                                    );
                                                }
                        ...
                        <div className="flex-1 flex flex-col min-w-0 relative h-full">
                                            ...
                                            <div key={member.userId} className="flex items-center gap-3 py-2 px-1 hover:bg-gray-50 rounded-lg">
                                                <div className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center overflow-hidden">
                                                    {member.avatar ? (
                                                        ...
                                                        <span className="text-xs font-bold text-gray-500 leading-none">{member.fullName.charAt(0)}</span>
                                                    )}
                                                </div>
                                                <div className="flex-1 min-w-0">
                            ...
                            <div className="flex-1 relative flex flex-col min-h-0">
                                ...
                                {showScrollBottom && unreadMessageCount > 0 && (
                                    ...
                                    >
                                        <ChevronDown size={28} className="text-gray-500 group-hover:text-orange-500 transition-colors" />
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <div className="py-8 flex flex-col items-center px-4">
                                                {(() => {
                                                    ...
                                                    if (avatars.length === 0) {
                                                        return (
                                                            <div className={`w-16 h-16 ${bgColorLight} rounded-full flex items-center justify-center mb-4 transition-transform hover:rotate-3 cursor-pointer`}>
                                                ...
                                                <h3 className="text-sm font-bold text-gray-800 text-center leading-tight mb-1 uppercase tracking-wider">{selectedGroup.name}</h3>
                                                <p className="text-gray-400 font-bold text-[10px] uppercase tracking-wide">Giảng viên: {selectedGroup.lecturerName || 'Mr. Alex'}</p>
                                            </div>

                                            {/* Actions List */}
                                            <div className="px-4 space-y-1">
                                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-2">Thông tin về đoạn chat</p>
                                                ...
                                                <div key={member.userId} className="flex items-center gap-3 group">
                                                    <div className="w-9 h-9 bg-gray-100 rounded-full flex items-center justify-center overflow-hidden border-2 border-white shadow-sm flex-shrink-0">
                                                        {member.avatar ? (
                                                            ...
                                                            <span className="text-sm font-bold text-gray-400 leading-none">{member.fullName.charAt(0)}</span>
                                                        )}
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <p className="font-bold text-gray-800 text-sm truncate">{member.fullName}</p>
                                                        <p className="text-[9px] text-gray-400 font-black uppercase tracking-widest">{member.role === 'LECTURER' ? 'Host' : 'Member'}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
