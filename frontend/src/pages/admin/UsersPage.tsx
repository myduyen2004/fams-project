...
export const UsersPage: React.FC = () => {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const data = await userService.getAllUsers({
        ...
        status: 'INACTIVE', // Hardcoded filter for this page
        page,
        size: 10
      });
      ...
      toast.error('Không thể tải danh sách người dùng chưa kích hoạt');
    } finally {
      setLoading(false);
    }
  ...
  const handleBulkActivate = async () => {
    ...
    const missingAvatar = selectedData.some(u => !u.avatar);

    if (missingAvatar) {
      toast.error('Tất cả tài khoản được chọn phải có ảnh đại diện mới có thể kích hoạt. Vui lòng cập nhật ảnh cho các tài khoản còn thiếu.');
      return;
    }
  ...
  const handleBulkDelete = async () => {
    if (!window.confirm(`Bạn có chắc chắn muốn xóa ${selectedUsers.length} người dùng đã chọn?`)) return;
    try {
      setIsDeleting(true);
      // We'll perform deletions sequentially or add a bulk delete API if available. 
      // For now, let's assume multiple calls if batch isn't ready, or just one if we have an API.
      // Based on userService.ts, we only have individual delete.
      ...
      <div className="p-6 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-800">
        ...
        {selectedUsers.length > 0 && (
          <div className="mb-4 p-3 bg-orange-50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-900/20 rounded-lg flex items-center justify-between animate-in fade-in slide-in-from-top-2">
            <span className="text-sm font-medium text-fpt-orange">Đã chọn {selectedUsers.length} tài khoản</span>
            <div className="flex gap-2">
               ...
               >
                 ...
                 {selectedUsers.length === 1 ? 'Xóa' : 'Xóa hàng loạt'}
               </button>
               <button 
               ...
               >
                 ...
                 {selectedUsers.length === 1 ? 'Kích hoạt' : 'Kích hoạt hàng loạt'}
               </button>
            </div>
          </div>
        )}
