...
import { Upload, Plus, Eye, Search, Filter, Loader2, ArrowLeft, X } from 'lucide-react';
...
export const MajorDetail = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            const [majorData, specsData] = await Promise.all([
                majorService.getMajor(parseInt(id)),
                specializationService.getSpecializationsByMajor(parseInt(id), {
                    keyword: searchTerm,
                    status: statusFilter // Removed generic ALL check
                })
            ]);
            setMajor(majorData);
            setSpecializations(specsData.content || []);
            setSelectedIds([]);
        } catch (error) {
                ...
                <div className="space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            {major && (
                                <div className="mt-2 text-sm text-gray-500 dark:text-gray-400 max-w-2xl">
                    ...
                    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                        <div className="mb-4 flex flex-col gap-4">
                            ...
                            {selectedIds.length > 0 && (
                                <>
                                    {selectedIds.some(id => specializations.find(s => s.id === id)?.status === 'ACTIVE') ? (
                                        <div className="mb-4 p-3 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 rounded-lg flex items-center justify-between animate-in fade-in slide-in-from-top-2">
                                            <span className="text-sm font-medium text-red-600">Đã chọn {selectedIds.length} chuyên ngành</span>
                                            <button
                                        ...
                                        <div className="mb-4 p-3 bg-green-50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/20 rounded-lg flex items-center justify-between animate-in fade-in slide-in-from-top-2">
                                            <span className="text-sm font-medium text-green-600">Đã chọn {selectedIds.length} chuyên ngành</span>
                                            <button
                        ...
                        <div className="flex items-center justify-between p-4 border-t border-gray-100 dark:border-zinc-800">
                            <div className="text-sm text-gray-500">
                                Hiển thị {specializations.length} chuyên ngành
                            </div>
                        </div>
                    </div>
...
// --- ImportSpecializationModal ---
const ImportSpecializationModal = ({ isOpen, onClose, majorId, onSuccess }) => {
                ...
                <form onSubmit={handleSubmit} className="p-4 space-y-4">
                    <div className="p-4 bg-blue-50 dark:bg-blue-900/10 text-blue-800 dark:text-blue-300 rounded-lg text-sm">
                        ...
                        <ul className="list-disc pl-4 space-y-1">
                            ...
                            <li>File cần có các cột: major code, specialization code, specialization name, description, totalCredits, status.</li>
                            <li>Chỉ các dòng có <strong>major code</strong> trùng với ngành hiện tại mới được import.</li>
                        </ul>
                    </div>
