                            ...
                            <div className="flex-1">
                                ...
                                {gradeOverview.gradesPublished && (
                                    <div className="flex items-center gap-2.5 px-4 py-2 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 text-[11px] font-bold uppercase tracking-wider rounded-xl border-2 border-green-100 dark:border-green-800/50 w-fit">
                                        ...
                                        <span>Đã công bố {gradeOverview.gradesPublishedAt ? new Date(gradeOverview.gradesPublishedAt).toLocaleDateString('vi-VN') : ''}</span>
                                        {gradeOverview.gradesPublishedBy && (
                                            <>
                                                <span className="opacity-40 font-normal">|</span>
                                                <span>{gradeOverview.gradesPublishedBy}</span>
                                            </>
                                        )}
                                    </div>
                                )}
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto">
                                {isEditMode ? (
                                    ...
                                    <>
                                        ...
                                        {gradeOverview.gradesPublished ? (
                                            <div className="flex h-[52px] items-center justify-center gap-2 px-6 bg-green-50 dark:bg-green-900/20 border-2 border-green-200 dark:border-green-800/50 rounded-2xl text-sm text-green-700 dark:text-green-400 font-bold">
                                                <ShieldCheck size={18} />
                                        ...
                                        {sortedGradeComponents.map((component) => (
                                            <th key={component.id} className={`px-2 py-3 text-center text-xs font-semibold uppercase tracking-wider min-w-[60px] ${component.isEditable ? 'bg-orange-600' : ''}`}>
                                                <div className="cursor-help flex items-center justify-center gap-1" title={`${component.name} (${component.weight}%)${component.isEditable ? ' - Có thể nhập điểm' : ' - Chỉ xem'}`}>
                                                    ...
                                                    {component.isEditable && <span className="text-[8px] bg-white/20 px-1 rounded">✎</span>}
                                                </div>
                                        ...
                                        return (
                                            <tr key={student.enrollmentId} className="hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors">
                                                ...
                                                <td className="px-4 py-2">
                                                    <button onClick={() => { setSelectedStudentCode(student.studentCode); setIsStudentInfoModalOpen(true); }} className="text-left font-bold text-gray-900 dark:text-white text-sm hover:text-fpt-orange transition-colors">
                                                        {student.studentName}
                                                    </button>
                                                </td>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[500] animate-in fade-in duration-300 p-4">
                    <div className="bg-white dark:bg-zinc-900 rounded-[32px] w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-300 border-2 border-gray-100 dark:border-zinc-800 overflow-hidden">
                        <div className="p-8">
                            ...
                            <h3 className="text-2xl font-black text-center text-gray-900 dark:text-white mb-3 tracking-tight">Xác nhận công bố điểm?</h3>
                            <div className="text-center space-y-4 mb-8">
                                <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed font-medium">
                                    Bạn đang công bố điểm cho môn <span className="font-bold text-gray-900 dark:text-white">{gradeOverview?.courseName}</span> - <span className="font-bold text-gray-900 dark:text-white">{gradeOverview?.semesterName}</span>.
                                </p>
                                <div className="bg-red-50 dark:bg-red-900/10 border-2 border-red-100 dark:border-red-900/20 rounded-2xl p-4 text-xs text-red-700 dark:text-red-400 text-left flex gap-3">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <p className="font-bold opacity-90">Sau khi công bố, sinh viên sẽ xem được điểm ngay lập tức.</p>
                                        <p className="font-black uppercase tracking-wide underline underline-offset-2">Hành động này không thể hoàn tác.</p>
                                    </div>
                                </div>
                            </div>
                            <div className="flex flex-col sm:flex-row gap-3">
                                <button onClick={() => setShowPublishConfirm(false)} className="flex-1 h-[52px] px-6 bg-white dark:bg-zinc-800 text-gray-700 dark:text-gray-300 rounded-2xl font-bold border-2 border-gray-100 dark:border-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-700 transition-all active:scale-95" disabled={publishing}>Hủy bỏ</button>
